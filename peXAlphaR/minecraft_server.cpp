...
SOCKET& operator>>(SOCKET& sck, varInt& dst) {
	...
	uint8_t i = 0;
	while ((tmp & VARINT_CONTINUE_BIT) && i < 5) {
...
int getServerInfo(const std::string& ip, uint16_t port, std::string& info) {
	...
	recv_vi >> recv_length;
	//recv json
	if (recv_length <= SOCKET_RECV_LENGTH_MAX) {
		char* buf = (char*)malloc(recv_length + 1);
		uint32_t tot_recv_bytes = 0, recv_bytes = recv_length;
		while (tot_recv_bytes < recv_length && recv_bytes != 0) {
